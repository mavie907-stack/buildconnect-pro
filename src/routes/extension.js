...
router.post('/posts', protect, wrap(async (req, res) => {
  const { title, content, body, type='update', images=[], tags=[] } = req.body;
  const text = content || body || title || '';
  if (!text) return fail(res, 'content is required');
  const data = {
    title: title||text.slice(0,80), content:text, type, images, tags,
