...
router.get('/rfps/:id/matches', protect, wrap(async (req, res) => {
  ...
  // Check cache first (safely — column may not exist yet)
  try {
  ...
  } catch(e) { /* column doesn't exist yet — skip cache */ }
...
Return exactly 5 matches (or fewer if less than 5 professionals exist). Order by score descending.`;
  ...
  // Cache on RFP record if possible (silently skip if columns don't exist)
  try {
    await sequelize.query(
      'UPDATE rfps SET ai_matches = :matches, ai_matches_at = :now WHERE id = :id',
      { replacements: { matches: JSON.stringify(finalMatches), now: new Date(), id: req.params.id } }
    );
