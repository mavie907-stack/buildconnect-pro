...
router.post('/rfps/:id/proposals', protect, wrap(async (req, res) => {
  ...
  await pushNotif(String(rfp.client_id), 'proposal',
...
router.post('/auth/avatar', protect, wrap(async (req, res) => {
  ...
  upload.single('avatar')(req, res, async (err) => {
    ...
    try {
      ...
      await User.update({ avatar: url }, { where: { id: uid(req) } });
...
router.post('/rfps/:id/matches/notify', protect, wrap(async (req, res) => {
  try {
    ...
    for (const m of matches) {
      try {
        await pushNotif(m.id, 'match', '🎯 New Project Match: ' + (rfp.title || ''), 'You matched a new project! ' + (m.reason || ''));
        notified++;
      } catch(e) {}
    }
