...
async function startServer() {
  try {
    ...
    // Safe sync — never alter existing tables (protects passwords & data)
    await sequelize.sync({ alter: false, force: false });
    console.log('✅ Database synchronized!');

    // Safely create new tables only
    const models = [Post, Message, Notification, Proposal];
    for (const model of models) {
      try { await model.sync({ alter: false, force: false }); } catch(e) {}
    }
    // Try to load optional models
    try { const LibraryFile = require('./models/LibraryFile'); await LibraryFile.sync({ alter: false, force: false }); } catch(e) {}
    try { const Portfolio   = require('./models/Portfolio');   await Portfolio.sync({ alter: false, force: false }); } catch(e) {}
    try { const Review      = require('./models/Review');      await Review.sync({ alter: false, force: false }); } catch(e) {}
