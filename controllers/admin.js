...
const getStats = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: { stats: { totalUsers, totalProjects, openProjects, draftProjects }, recentUsers, recentProjects },
    });
  } catch (error) {
...
const getAllUsers = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: { message: 'Failed to load users' } });
  }
};
...
const getUserDetails = async (req, res) => {
  try {
    const user = await User.findByPk(req.params.id, {
      attributes: { exclude: ['password'] },
      include: [{ model: RFP, as: 'rfps', limit: 10, order: [['createdAt', 'DESC']] }],
    });

    if (!user) return res.status(404).json({ success: false, error: { message: 'User not found' } });
    res.json({ success: true, data: user });
  } catch (error) {
    res.status(500).json({ success: false, error: { message: 'Failed to load user details' } });
  }
};
...
const updateUser = async (req, res) => {
  try {
    ...
    res.json({ success: true, data: user.toPublicJSON(), message: 'User updated successfully' });
  } catch (error) {
...
const deleteUser = async (req, res) => {
  try {
    const user = await User.findByPk(req.params.id);
    if (!user) return res.status(404).json({ success: false, error: { message: 'User not found' } });
    if (user.id === req.userId) return res.status(400).json({ success: false, error: { message: 'Cannot delete your own account' } });
...
const getAllProjects = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: { message: 'Failed to load projects' } });
  }
};
...
const updateProjectStatus = async (req, res) => {
  try {
    ...
    if (!rfp) return res.status(404).json({ success: false, error: { message: 'Project not found' } });

    const { status, featured } = req.body;
    await rfp.update({ status, featured });

    res.json({ success: true, data: rfp, message: 'Project updated successfully' });
  } catch (error) {
    res.status(500).json({ success: false, error: { message: 'Failed to update project' } });
  }
};
...
const globalSearch = async (req, res) => {
  try {
    const { q } = req.query;
    if (!q || q.length < 2) return res.json({ success: true, data: { users: [], projects: [] } });

    const users = await User.findAll({
      where: { [Op.or]: [{ name: { [Op.iLike]: `%${q}%` } }, { email: { [Op.iLike]: `%${q}%` } }, { company: { [Op.iLike]: `%${q}%` } }] },
      limit: 10,
      attributes: { exclude: ['password'] },
    });
    ...
    res.json({ success: true, data: { users, projects, query: q, totalResults: users.length + projects.length } });
  } catch (error) {
