...
// Fetch all active users with their portfolio data — never specify column list
// so it works even before migration adds portfolio_projects column
async function fetchUsersWithPortfolios() {
  const User = tryModel('../models/User');
  if (!User) return [];
  try {
    // Exclude password only — get everything else including portfolio_projects once it exists
