  ...
  if (!projects.length && User) {
    try {
      ...
      users.forEach(u => {
        ...
        projs.forEach(p => {
          if(!p.title) return;
          if(p.is_public === false) return;   // respect privacy setting
  ...
  if (User) {
    try {
      ...
      users.forEach(u => {
        ...
        let projs = [];
        try { projs = JSON.parse(uData.portfolio_projects || '[]'); } catch(e) {}
        projs.filter(p=>p.featured && p.title && p.is_public !== false).forEach(p => {
          featured.push({
            ...p,
            user: { id:uData.id, name:uData.name, company:uData.company, role:uData.role, avatar:uData.avatar },
          });
  ...
  if (User) {
    try {
      ...
      users.forEach(u => {
        ...
        let projs = [];
        try { projs = JSON.parse(uData.portfolio_projects || '[]'); } catch(e) {}
        const publicProjs = projs.filter(p => p.title && p.is_public !== false);
        if(publicProjs.length){ proCount++; projCount += publicProjs.length; }
        publicProjs.forEach(p=>{ if(p.location) countries.add(p.location.split(',').pop().trim()); if(p.type) types.add(p.type); });
      });
