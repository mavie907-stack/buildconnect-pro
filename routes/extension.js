...
async function getUserPortfolio(userId) {
  ...
  try {
    const result = await sequelize.query(
      `SELECT portfolio_projects FROM "users" WHERE id = :id LIMIT 1`,
      { replacements: { id: userId } }
    );
    const row = result[0]?.[0];
    if (row?.portfolio_projects) {
      const raw = row.portfolio_projects;
      return JSON.parse(typeof raw === 'string' ? raw : JSON.stringify(raw));
    }
  } catch(e) { console.error('[portfolio] getUserPortfolio error:', e.message); }
  return [];
}
...
async function fetchPortfolioUsers() {
  ...
  if (!sequelize) return [];
  try {
    // No WHERE filter — get ALL users who have portfolio data
    const result = await sequelize.query(
      `SELECT id, name, company, role, avatar, location, portfolio_projects FROM "users" WHERE portfolio_projects IS NOT NULL AND portfolio_projects != '[]' AND portfolio_projects != ''`
    );
    const rows = Array.isArray(result[0]) ? result[0] : [];
    console.log(`[gallery] fetchPortfolioUsers: found ${rows.length} users with portfolio data`);
    return rows;
  } catch(e) {
    console.error('[gallery] fetchPortfolioUsers error:', e.message);
    return [];
  }
}
...
router.get('/public/gallery', wrap(async (req, res) => {
  ...
  users.forEach(u => {
    ...
    console.log(`[gallery] user ${u.name}: ${projs.length} projects`);
    projs.forEach(p => {
      if (!p || !p.title) return;
  ...
  console.log(`[gallery] returning ${total} projects`);
