...
// ─── Load models ───────────────────────────────────────────────────
let User, Message, Post, Notification, Comment, Follow, Event;
...
//  LIBRARY FILES
// ═══════════════════════════════════════════════════════════════════
router.get('/library', (req, res) => { ok(res, { files: _library, items: _library }); });
router.post('/library', protect, adminOnly, (req, res) => {
  const item = { id: Date.now().toString(), ...req.body, createdAt: new Date() };
  _library.unshift(item);
  ok(res, item);
});
router.put('/library/:id', protect, adminOnly, (req, res) => {
...
router.delete('/library/:id', protect, adminOnly, (req, res) => {
...
router.get('/portfolio', protect, wrap(async (req, res) => {
  ...
  const posts  = _posts.filter(p => String(p.author_id)===userId);
  ok(res, { portfolio:{ user, posts, items:[] } });
}));
...
try {
  ...
  upload = multer({ storage, limits:{ fileSize: 10*1024*1024 } }); // 10MB
} catch(e) {
  console.warn('[extension] multer not available — file uploads disabled');
}

router.post('/posts/upload', protect, (req, res, next) => {
  if (!upload) return ok(res, { files:[] }, 'Upload disabled — multer not installed');
