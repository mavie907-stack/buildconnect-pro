...
// Helper: save portfolio array to user
async function saveUserPortfolio(userId, projects) {
  try {
    if (User && isSeq) {
      await User.update(
        { portfolio_projects: JSON.stringify(projects) },
        { where: { id: userId } }
      );
      return true;
    }
...
// Library file upload (PDF/image direct to Cloudinary)
...
(async () => {
  ...
  try {
    ...
    if (tableDesc && !tableDesc.portfolio_projects) {
      await qi.addColumn('Users', 'portfolio_projects', {
        type: sequelize.constructor.DataTypes.TEXT,
        allowNull: true,
        defaultValue: null,
      });
      console.log('✅ [migration] Added portfolio_projects column to Users');
    }
  } catch(e) {
    console.warn('[migration] portfolio_projects column check skipped:', e.message);
  }
})();
